...
def create_data_splits(for_asn_training):
    ...
    child_df = pd.read_csv(
        "/workspaces/ASN/data/parent_relation/celebtrity_dataset_split.csv"
    )

    # read in the data
    df = pd.read_csv(
        "/workspaces/ASN/data/parent_relation/celebrity_relations_parent_child_pairs.csv"
    )
    ...
    for _, row in df.iterrows():
        ...
        if (
            child_df.loc[child_df["child"] == child]["dataset"].item() == "train"
        ):  # for some add both to train
            ...
            if child_df.loc[child_df["child"] == child]["direction"].item():
                ...
                child_parent_pairs_seen.append(
                    (child, parent, reverse_relation, sex_parent)
                )

                if (
                    for_asn_training
                ):  # if we use SLASH training we obtain the other way around as well
                    train_data.append((child, parent, reverse_relation, sex_parent))
            else:
                train_data.append((child, parent, reverse_relation, sex_parent))
                parent_child_pairs_seen.append(
                    (parent, child, relation, sex_lookup[child])
                )
                if (
                    for_asn_training
                ):  # if we use SLASH training we obtain the other way around as well
            ...
            child_df.loc[child_df["child"] == child]["dataset"].item() == "val"
        ):  # for some add both to train
            val_data.append((child, parent, reverse_relation, sex_parent))
            val_data.append((parent, child, relation, sex_lookup[child]))

        elif (
            child_df.loc[child_df["child"] == child]["dataset"].item() == "test"
        ):  # for some add only child parent to train and test the other way around
            parent_child_pairs_unseen.append(
                (parent, child, relation, sex_lookup[child])
            )
            child_parent_pairs_unseen.append(
                (child, parent, reverse_relation, sex_parent)
            )
    ...
    return (
        train_data,
        val_data,
        child_parent_data_seen,
        parent_child_data_seen,
        child_parent_data_unseen,
        parent_child_data_unseen,
    )
...
class FamilyRelationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_text = (
            f"What is the relation between {person1} and {person2}? The answer is:"
        )
...
class FamilyRelationASNDataset(Dataset):
    ...
    def __init__(self, data, tokenizer, split, max_length=128):
        ...
        self.max_length = max_length
        self.split = split
        self.class_token_ids_relation = torch.tensor(
            [
                tokenizer.encode(c, add_special_tokens=False)[0]
                for c in ["mother", "father", "daughter", "son"]
            ]
        )
        self.class_token_ids_sex = torch.tensor(
            [
                tokenizer.encode(c, add_special_tokens=False)[0]
    ...
    def __getitem__(self, idx):
        ...
        input_text_derived = (
            f"What is the relation between {person2} and {person1}? The answer is:"
        )
        ...
        inputs_sex_person1 = self.tokenizer(
            input_text_sex_person1,
            return_tensors="pt",
            max_length=self.max_length,
            padding="max_length",
            truncation=True,
        ...
        inputs_sex_person2 = self.tokenizer(
            input_text_sex_person2,
            return_tensors="pt",
            max_length=self.max_length,
            padding="max_length",
            truncation=True,
...
def evaluate_model(
    model, test_loader, tokenizer, device, batch_size=8, print_preds=False
):
    """
    ...
    relation_token_ids = [
        tokenizer.encode(rel, add_special_tokens=False)[0] for rel in relations
    ]
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Evaluating"):
            ...
            logits = outputs.logits[
                :, next_token_pos, :
            ]  # Get logits for the last token -> get the logits at the end of the input text
            ...
            predictions = [
                relations[i] for i in predicted_relation_indices.cpu().numpy()
            ]
    ...
    class_precision, class_recall, class_f1, _ = precision_recall_fscore_support(
        all_labels, all_preds, zero_division=0, average=None
    )
