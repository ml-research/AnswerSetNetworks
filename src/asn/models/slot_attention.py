...
def spatial_broadcast(slots, resolution):
    ...
    slots = torch.reshape(
        slots, [slots.shape[0] * slots.shape[1], 1, 1, slots.shape[2]]
    )

    grid = slots.repeat(
        1, resolution[0], resolution[1], 1
    )  # repeat expands the data along differnt dimensions
...
class SlotAttention(nn.Module):
    def __init__(self, num_slots, dim, iters=3, eps=1e-8, hidden_dim=128):
        ...
        self.slots_mu = nn.Parameter(
            torch.randn(1, 1, dim)
        )  # randomly initialize sigma and mu
        self.slots_log_sigma = (
            nn.Parameter(torch.randn(1, 1, dim)).abs().to(device="cuda")
        )
    ...
    def forward(self, inputs, num_slots=None):
        b, n, d = (
            inputs.shape
        )  # b is the batchsize, n is the dimensionsize of the features, d is the amount of features([15, 1024, 32])
        ...
        for _ in range(self.iters):
            ...
            )  # is M in the paper, has shape 1024(feature map)| 7(slot amount)
            attn = (
                dots.softmax(dim=1) + self.eps
            )  # calcualte the softmax for each slot which is also 1024 * 7
    ...
    def __init__(self, in_channels, hidden_channels, clevr_encoding):
        ...
        if clevr_encoding:
            self.network = nn.Sequential(
                nn.Conv2d(
                    in_channels, hidden_channels, (5, 5), stride=(1, 1), padding=2
                ),
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    hidden_channels, hidden_channels, (5, 5), stride=(2, 2), padding=2
                ),
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    hidden_channels, hidden_channels, (5, 5), stride=(2, 2), padding=2
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    hidden_channels, hidden_channels, (5, 5), stride=(1, 1), padding=2
                ),
                nn.ReLU(inplace=True),
            )
        else:
...
class SoftPositionEmbed(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return inputs + self.dense(self.grid).view(
            (-1, self.hidden_size, self.resolution, self.resolution)
        )
...
class SlotAttention_classifier(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.network = nn.Sequential(
            ...
            ),  # nn.Conv1d(in_channels, in_channels, 1, stride=1, groups=in_channels)
            nn.ReLU(inplace=True),
