...
# import EinsumNetworks
# import EinsumNetworks.src
# from EinsumNetworks.src import EinsumNetwork
...
class EiNet(EinsumNetwork.EinsumNetwork):
    def __init__(
        ...
        if self.structure == "poon-domingos":
            pd_delta = [
                [self.pd_height / d, self.pd_width / d] for d in self.pd_num_pieces
            ]
            graph = Graph.poon_domingos_structure(
                shape=(self.pd_height, self.pd_width), delta=pd_delta
            )
